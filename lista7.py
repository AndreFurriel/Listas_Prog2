...
#Questão 3
"""
Estenda o código CodigosExcerciciosProvas/poly_interpolant.py com uma classe de interpolante que construa o polinômio de Lagrange
usando o Scipy.
...
class LagrangePolinomial(interpolater):
    def __init__(self, x, y):
        if len(x) != len(y):
            raise RuntimeError(f"Dimensions must be equal len(x) = {len(x)} != len(y) = {len(y)}")
        self.data = [x, y]
